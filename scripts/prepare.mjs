...
if (hostBuilt && clientBuilt) {
  console.log('[dsh-mcpmanager] 构建产物已存在（lib/ + client/），跳过 prepare 构建')
  process.exit(0)
}

console.log('[dsh-mcpmanager] 未发现构建产物，执行构建…')
const nodeBin = process.execPath

function run(label, entry, args) {
  const r = spawnSync(nodeBin, [entry, ...args], { stdio: 'inherit' })
  if (r.status !== 0) {
    console.error(`[dsh-mcpmanager] ${label} failed (exit ${r.status})`)
